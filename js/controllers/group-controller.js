ndexApp.controller('groupController',
    ['ndexService', 'ndexUtility', 'sharedProperties', '$scope', '$location', '$routeParams', '$modal',
        function (ndexService, ndexUtility, sharedProperties, $scope, $location, $routeParams, $modal) {
            ...
            //user
            $scope.user.currentGroupName = $routeParams.groupName;

            // called on Networks belonging to group displayed on page
            $scope.group.setAndDisplayCurrentNetwork = function (networkId) {
                sharedProperties.setCurrentNetworkId(networkId);
                $location.path("/networkQuery/" + networkId);
            };

            $scope.user.setAndDisplayCurrentUser = function (userName) {
                sharedProperties.setCurrentUserName(userName);
            ...
            ndexService.getGroup($scope.group.currentGroupName)
                .success(
                function (group) {
